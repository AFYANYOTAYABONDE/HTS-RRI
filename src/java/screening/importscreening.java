 ...
 * @author Emmanuel E
 */
public class importscreening extends HttpServlet {
...
System.out.println(" Workload Data upload by "+counselorname+" ");
   ...
   //,newart=?,newpos_pmtct=?,art_pmtct=?
          ...
          insert=" update workload set facility=? , counselorname=? , date_tested=? , opd_t=? , screened=? , screenrate=? , opd_eligible=? , opd_tested=? , opd_positive=? , opd_Yield=? , timestamp=? , user=?   "
...
conn.pst1.setString(11,timestamp);
conn.pst1.setString(12,user);
                        ...
                        { 
                            ...
                            if(counselorname.equals("select counsellor")){
                                try {
                                    ...
                                    Logger.getLogger(importscreening.class.getName()).log(Level.SEVERE, null, ex);
                                }
                            }
...
//     // System.out.println(myresponse);                   
          ...
          insert=" insert into workload(id,facility,counselorname,date_tested,opd_t,screened,screenrate,opd_eligible,opd_tested,opd_positive,opd_Yield,timestamp,user) "
                 + " values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
                      conn.pst1=conn.conne.prepareStatement(insert);    
...
conn.pst1.setString(12,timestamp);
conn.pst1.setString(13,user);
    ...
    if(conn.rs!=null){conn.rs.close();}
        ...
        //send an email at this point of the exception
            ...
            try {
                ...
                Logger.getLogger(importscreening.class.getName()).log(Level.SEVERE, null, ex1);
            }
